...
final class FaqEmotionEngine
{
    ...
    private static function boostRules(): array
    {
        return [
            ...
            ['worried', '/\b(worri|concerned|nabalaka|kabalaka|alala)\b/ui', 2.0],
            ['anxious', '/\b(anxious|anxiety|ginakulbaan|kulba)\b/ui', 2.0],
            ['panic', '/\b(panic|ginapanik|buligi\s+ko|help\s+me\s+now)\b/ui', 2.5],
            ['sad', '/\b(sad|lungkot|kasubo|subo|malungkot|nalain)\b/ui', 2.0],
            ['sad', '/\b(pamatyag|nararamdaman).*(malain|kasubo)\b/ui', 2.2],
            ['tired', '/\b(tired|pagod|kapoy|wala\s+na\s+(ko\s+)?kusog)\b/ui', 2.0],
            ['stressed', '/\b(stress|stressed|overwhelm|grabeng\s+stress)\b/ui', 2.0],
            ['lonely', '/\b(lonely|nag-iisa|isa\s+lang|wala\s+(ako|ko)\s+(makakausap|maistoryahan))\b/ui', 2.0],
            ['thankful', '/\b(salamat|thank\s*you|thanks|maraming\s+salamat)\b/ui', 2.5],
            ['hopeless', '/\b(hopeless|walang\s+pag-asa|wala\s+paglaum|wala\s+na\s+solusyon|ayaw\s+ko\s+mabuhay|going\s+to\s+die|gonna\s+die|im\s+going\s+to\s+die|i\'?m\s+going\s+to\s+die)\b/ui', 3.0],
            ['afraid', '/\b(afraid|scared|fearful|nahadlok|natakot|takot)\b/ui', 2.2],
            ['disappointed', '/\b(disappoint|nadismaya|dismaya)\b/ui', 2.0],
            ['nervous', '/\b(nervous|kinakabahan|kabado)\b/ui', 2.0],
            ['pain', '/\b(sakit\s+ulo|headache|masakit|sakit\s+lawas)\b/ui', 2.5],
            ['sick', '/\b(hilanat|lagnat|fever|sick|sipon|ubo|gasuka|may\s+sakit)\b/ui', 2.5],
            ['crying', '/\b(crying|umiiyak|naga\s*hilib|naga\s*hibi)\b/ui', 2.0],
            ['overwhelmed', '/\b(overwhelm|overwhelmed|daw\s+wala\s+na\s+ko\s+gana|wala\s+na\s+akong\s+gana)\b/ui', 2.0],
            ['happy', '/\b(happy|masaya|masadya|sayo)\b/ui', 2.0],
            ['confused', '/\b(confus|nalibog|nalilito|libog)\b/ui', 2.0],
            ['emergency', '/\b(heart\s+attack|stroke|can\'?t\s+breathe|hindi\s+makahinga|dili\s+makaginhawa|unconscious|nawalan\s+ng\s+malay)\b/ui', 3.5],
        ];
    }
