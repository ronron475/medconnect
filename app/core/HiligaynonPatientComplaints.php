<?php
/**
 * Loads data/nlp/hiligaynon_patient_complaints.csv — realistic telemedicine patient complaints.
 */

final class HiligaynonPatientComplaints
{
    ...
    {
        return BASE_PATH . '/data/nlp/hiligaynon_patient_complaints.csv';
    }
    ...
    public static function rows(): array
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if ($mapped['patient_complaint_hiligaynon'] !== '') {
                self::$rows[] = $mapped;
            }
        }
        fclose($handle);
    ...
    private static function mapRow(array $header, array $row): array
    {
        ...
        return [
            'id'                           => (string) ($data['id'] ?? ''),
            'patient_complaint_hiligaynon' => (string) ($data['patient_complaint_hiligaynon'] ?? ''),
    ...
    public static function complaintIndex(): array
    {
        ...
        foreach (self::rows() as $row) {
            ...
            $terms = [$row['patient_complaint_hiligaynon'], $row['normalized_symptom']];
    ...
    {
        $key = self::normalizeComplaint($complaint);

        return self::complaintIndex()[$key] ?? null;
    }

    public static function translateComplaint(string $text): string
    {
        $key = self::normalizeComplaint($text);
        $entry = self::complaintIndex()[$key] ?? null;

        return $entry !== null ? (string) ($entry['english'] ?? '') : '';
    }

    /**
     * Match longest complaint phrases inside free-form patient text.
     */
