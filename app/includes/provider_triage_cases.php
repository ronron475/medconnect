...
/**
 * Load triage cases visible to a provider (patients with consultations or booked slots).
 *
 ...
 * @return array{total: int, urgent: int, non_urgent: int, reviewed: int, pending: int}
 */
function provider_triage_cases_stats(array $cases): array
{
    $urgent     = count(array_filter($cases, fn($t) => ($t['urgency'] ?? '') === 'Urgent'));
    $reviewed   = count(array_filter($cases, fn($t) => !empty($t['reviewed'])));
    $total      = count($cases);
