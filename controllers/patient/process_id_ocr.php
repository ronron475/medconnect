...
if (!$entered_first || !$entered_last || !$entered_dob || !$entered_id) {
    echo json_encode(['success' => false, 'message' => 'Please fill in all required fields before verifying your ID.']);
    exit;
}

if (empty($_FILES['national_id_image']) || $_FILES['national_id_image']['error'] === UPLOAD_ERR_NO_FILE) {
    echo json_encode(['success' => false, 'message' => 'No ID image uploaded.']);
    exit;
}
...
if ($file['error'] !== UPLOAD_ERR_OK) {
    ...
    echo json_encode(['success' => false, 'message' => $upload_errors[$file['error']] ?? 'Unknown upload error.']);
    exit;
}
...
$upload_dir = __DIR__ . '/../../storage/uploads/ids/';
...
if ($ocr === null) {
    cleanTemp($processed);
    echo json_encode(['success' => false, 'message' => 'Could not reach the OCR service. Please check your connection and try again.']);
    exit;
}
if ($ocr_fail) {
    cleanTemp($processed);
    echo json_encode(['success' => false, 'message' => ocrFriendlyError($ocr)]);
    exit;
}
...
if ($parsed_text_e2 === '') {
    cleanTemp($processed);
    echo json_encode(['success' => false, 'message' => 'No text could be extracted from the uploaded ID. Please upload a clearer photo.']);
    exit;
}
...
// ── Final decision: verified | manual_review | failed ────────
// Hard errors = name/DOB/clear-ID-fail/clear-residency-fail
// Soft flags  = id_state=fuzzy OR bago_state=fallback (these do NOT go into $errors)
...
if ($id_state === 'fuzzy') {
    $manual_review_reasons[] = 'National ID number could not be read with full confidence (OCR may have misread 1–2 digits).';
}
if ($id_state === 'fail' && $extracted_id_candidate !== '') {
    $manual_review_reasons[] = 'National ID number OCR could not confirm an exact match — admin will verify the stored ID image.';
}
if ($bago_state === 'fallback') {
    $manual_review_reasons[] = 'Bago City residency was inferred from address clues — the word "Bago" was not directly detected.';
}

// Promote to manual_review when:
// - no hard errors exist (names/DOB/clear-fail are all clean)
// - at least one soft flag is raised (fuzzy ID or fallback residency)
if ($names_dob_pass && empty($errors) && !empty($manual_review_reasons)) {
    $needs_manual_review = true;
}

// If names, DOB, and Bago City all pass — grant verified regardless of ID OCR result.
// The stored ID image is available for admin review.
if ($names_dob_pass && $bago_found && empty($errors)) {
    $needs_manual_review = false; // promote to verified
}

// Determine final state
if (empty($errors) && !$needs_manual_review) {
    $final_state = 'verified';
} elseif ($needs_manual_review) {
...
echo json_encode([
    ...
    'verified'             => $verified,
    'final_state'          => $final_state,
    'manual_review'        => ($final_state === 'manual_review'),
    'manual_review_reasons'=> $manual_review_reasons,
