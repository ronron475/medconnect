...
$admin_initials = profile_picture_initials($_SESSION['first_name'] ?? 'A', $_SESSION['last_name'] ?? '');
$admin_picture_url = profile_picture_public_url($_SESSION['profile_picture'] ?? null);

$nav_sections = require BASE_PATH . '/app/includes/nav/admin_nav.php';

function admin_nav_is_active(string $file, string $current, string $query, ?string $itemQuery): bool
{
...
<aside class="adm-sidebar">
  ...
  <a href="<?= ASSET_BASE ?>/views/admin/dashboard.php" class="adm-logo">
    ...
    <div class="adm-logo-text">med<span>Connect</span><em>Admin</em></div>
  </a>
  ...
  <nav class="adm-nav">
    ...
    <?php endif;
      foreach ($section['items'] as $item):
        ...
        $href = ASSET_BASE . '/views/admin/' . $file . ($itemQuery ? '?' . $itemQuery : '');
        $is_active = admin_nav_is_active($file, $current, $current_query, $itemQuery);
    ?>
  ...
  <a href="<?= ASSET_BASE ?>/views/admin/profile.php" class="adm-profile" title="Administrator profile settings">
    ...
    <div class="adm-profile-info">
      ...
      <div class="adm-profile-role">System Administrator</div>
    </div>
  </a>
